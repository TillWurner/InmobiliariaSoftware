...
@section('contenido')
    ...
    <div class="container scrollable-container">
        <div class="row">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            @if ($inmueble->id_asesor == null)
                                                ...
                                                <form id="formulario" method="POST" {{-- action="{{ route('asignarAsesor', $inmueble->id) }}" --}} action="#"
                                                    enctype="multipart/form-data">
                                                    @csrf
            ...
            @endrole()
            @role('asesor')
                @if ($inmueblesA->isEmpty())
                    <div class="lista" style="color: rgb(253, 173, 0); font-size: 50px; margin-top: 90px">
                        NO TIENES INMUEBLES ASIGNADO !!!
                    </div>
                @else
                    @foreach ($inmueblesA as $i => $inmueble)
                        @php
                            $inmueble->imagen = $inmueble->imagenes->where('destacado', true)->first();
                        @endphp
                        <div class="col-md-4">
                            <div class="image-container">
                                @if ($inmueble->imagen)
                                    <img src="{{ asset('fotos/fotos-inmuebles/' . $inmueble->imagen->imagen) }}"
                        ...
                        <div class="modal" id="modalMod{{ $loop->iteration }}" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form class="container" method="POST"
                                        action="{{ route('modificarInmueble', $inmueble->id) }}"
                                        enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                <label for="recipient-name" class="col-form-label">Direccion:</label>
                                                <input name="direccion" type="text" class="form-control"
                                                    id="recipient-name" maxlength="20" value="{{ $inmueble->direccion }}"
                                                    required>
                                            </div>
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label for="tamaño" class="col-form-label">Tamaño (mts²):</label>
                                                        <input name="tamano" type="text" class="form-control"
                                                            id="tamaño" maxlength="10" value="{{ $inmueble->tamano }}"
                                                            required>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label for="tamaño" class="col-form-label">Precio ($us):</label>
                                                        <input name="precio" type="text" class="form-control"
                                                            id="tamaño" maxlength="10" value="{{ $inmueble->precio }}"
                                                            required>
                                                    </div>
                                                </div>
                                            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form class="container" method="POST" action="{{ route('registrarInmueble') }}"
                    ...
                    <div class="modal-footer">
                        <button id="guardar-btn" type="submit" class="btn btn-success">
                            Guardar
                        </button>
                        <button type="button" class="btn btn-danger" data-dismiss="modal">Cerrar</button>
                    </div>
    ...
    <script type="module">
        ...
        document.querySelectorAll('.btn-aceptar-asesor').forEach(function(button) {
            ...
            guardarBtn.addEventListener('click', function(event) {
                ...
                // Obtener el valor del input de coordenada
                // const coordenada = document.getElementById('coordenada-input').value;

                // Hacer lo que necesites con la coordenada (por ejemplo, enviarla al servidor)
                socket.emit("coordenadas-inmueble", {
                    coordenada_inmueble: 1,
                });
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Leer el estado almacenado del LocalStorage
            const modalState = localStorage.getItem('modalState');
            if (modalState) {
                const data = JSON.parse(modalState);

                // Colocar los datos almacenados en el modal
                // Por ejemplo, puedes usar los datos para rellenar los campos del modal

                // Obtener el ID del modal correspondiente a los datos almacenados
                const modalId = data.id_inmueble;

                // Abre el modal utilizando el ID
                $(`#modal${modalId}`).modal('show');
            }

            // Limpiar el estado almacenado del LocalStorage después de usarlo
